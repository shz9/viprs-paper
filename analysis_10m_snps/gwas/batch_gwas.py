    ...
    if config == 'real':
        for keep_f in glob.glob(osp.join(keepfile_dir, 'ukbb_cv', args.type, trait, "*/train.keep")):
            ...
            jobs.append({
                'Trait': trait_f,
                'Keep': keep_f,
                'Name': f"{args.type}/{config}_{fold}/{trait}",
                'Output': f"data_all/gwas/{args.type}/{config}_{fold}/{trait}"
            })
    else:
        jobs.append({
            'Trait': trait_f,
            'Keep': osp.join(keepfile_dir, "ukbb_train_subset.keep"),
            'Name': f"{args.type}/{config}/{trait}",
            'Output': f"data_all/gwas/{args.type}/{config}/{trait}"
        })
...
for job in jobs:
    ...
    makedir(osp.dirname(f"./log_all/{job['Name']}.out"))

    try:
        os.remove(f"./log_all/{job['Name']}.out")
    except Exception as e:
        pass

    cmd = ["sbatch", "-J", job['Name'], "analysis_10m_snps/gwas/plink_gwas.sh", job['Trait'], job['Keep'], job['Output']]
    print(" ".join(cmd))
