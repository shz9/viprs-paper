...
parser.add_argument('-l', '--panel', dest='panel', type=str, default='ukbb_all_windowed',
                    choices={'external', '1000G_sample', '1000G_shrinkage', '1000G_windowed', '1000G_block',
                             ...
                             'all', 'ukbb_all_windowed'})

args = parser.parse_args()

model_fit_dir = "data_all/model_fit/"
...
for mdir in glob.glob(model_fit_dir):
    ...
    jobs.append({
        'Trait': mdir,
        'Name': mdir.replace('data_all/model_fit/', '')
    })
...
for job in jobs:
    ...
    makedir(osp.dirname(f"./log_all/score/{job['Name']}.out"))

    try:
        os.remove(f"./log_all/score/{job['Name']}.out")
    except Exception as e:
        pass

    cmd = ["sbatch", "-J", job['Name'], "analysis_10m_snps/score/score_job.sh", job['Trait']]
    print(" ".join(cmd))
