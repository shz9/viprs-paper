...
n_folds = 5
validation_prop = 0.1  # Proportion of training samples used for validation
ind_keep_file = "data/keep_files/ukbb_qc_individuals.keep"
phenotype_dir = "data/phenotypes/real/"
output_dir = "data/keep_files/ukbb_cv/"

print("> Generating Cross-Validation folds for real phenotypes.")
...
for pheno_file in glob.glob(osp.join(phenotype_dir, "*.txt")):
    ...
    for i, (train_idx, test_idx) in enumerate(kf.split(phe_df), 1):
        ...
        test_subset = phe_df.iloc[test_idx]
        train_subset, validation_subset = train_test_split(phe_df.iloc[train_idx], test_size=validation_prop)
