...
dir.create(sprintf("data/model_fit/external/LDPred2-inf/%s/%s/%s/", trait_type, config, trait),
           showWarnings = FALSE,
           recursive = TRUE)
...
for (chr in 1:22) {
  ...
  write.table(chr_sumstats,
              sprintf("data/model_fit/external/LDPred2-inf/%s/%s/%s/chr_%d.fit", trait_type, config, trait, chr),
              row.names = F,
              sep = "\t")
}
