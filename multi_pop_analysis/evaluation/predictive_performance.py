...
def process_trait(trait_f):
    ...
    for prs_file in glob.glob(f"data/minority_test_scores/{search_panel}/{search_model}"
        ...
        for anc in ancestry_df['Ancestry'].unique():

            anc_merged_df = merged_df.merge(ancestry_df.loc[ancestry_df['Ancestry'] == anc, ], on=['FID', 'IID'])

            try:
                if trait_type == 'binary':
                    res = evaluate_binomial_predictive_performance(anc_merged_df)
                else:
                    res = evaluate_gaussian_predictive_performance(anc_merged_df)
    ...
    output_file = f"data/evaluation_minority/{trait_type}/{config}/{trait}.csv"
    ...
    eval_pheno_df.to_csv(f"data/evaluation_minority/{trait_type}/{config}/{trait}.csv", index=False)


if __name__ == '__main__':

    parser = argparse.ArgumentParser(description='Evaluate the predictive performance of '
                                                 'PRS models on minority populations')
    ...
    if args.type == 'all':
        pheno_dir = f"data/phenotypes_minority/*"
    else:
        pheno_dir = f"data/phenotypes_minority/{args.type}"
    ...
    ancestry_df = pd.read_csv("data/covariates/ukbb_individual_ancestry.keep", delim_whitespace=True)
